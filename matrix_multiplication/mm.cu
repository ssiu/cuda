...
__global__ void basic_mm(float* A, float* B, float* C, int N) {
    ...
    for (int TILE = 0; TILE < WIDTH/INNER_TILE_WIDTH; TILE++) {
        ...
        int offset_A = 1;//TILE * INNER_TILE_WIDTH / SHRINK_FACTOR;// + blockDim.y * blockIdx.y * OUTER_TILE_WIDTH * WIDTH / SHRINK_FACTOR;
        int offset_B = 1;//TILE * INNER_TILE_WIDTH / SHRINK_FACTOR;// + blockDim.x * blockIdx.x * OUTER_TILE_WIDTH * WIDTH / SHRINK_FACTOR;
