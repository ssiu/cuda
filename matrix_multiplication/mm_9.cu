...
mm_9(float* A, float* B, float* C, int N){
    ...
    reinterpret_cast<float4*>(sB[0])[(sB_row * TILE_WIDTH + sB_col) >> 2] = reinterpret_cast<float4*>(B)[(sB_row * N + gB_col) >> 2];
    ...
    for (int kBlock = 0; kBlock < N / BLOCK_WIDTH - 1; kBlock++){
        ...
        //reinterpret_cast<float4*>(sA)[(sA_row * BLOCK_WIDTH + sA_col) / 4] = reinterpret_cast<float4*>(A)[(gA_row * N + gA_col) / 4];
        // no bank conflict for B
        reinterpret_cast<float4*>(sB[(kBlock + 1) & 1])[(sB_row * TILE_WIDTH + sB_col) >> 2] = reinterpret_cast<float4*>(B)[(gB_row * N + gB_col) >> 2];
...
//                fragment_A[i+4] = sA[(warp_row + thread_row + 16 + i) * BLOCK_WIDTH + kFragment];
                ...
                //
                fragment_A[i] = sA[kBlock & 1][(warp_row + thread_row + i) * BLOCK_WIDTH + ((thread_row >> 2) + ((kFragment & 3)) & 3) + (kFragment >> 2) * 4];
                fragment_A[i+4] = sA[kBlock & 1][(warp_row + thread_row + 16 + i) * BLOCK_WIDTH + (((thread_row >> 2) + (kFragment & 3)) & 3) + (kFragment >> 2) * 4];
...
//                fragment_A[i+4] = sA[(warp_row + thread_row + 16 + i) * BLOCK_WIDTH + kFragment];
                ...
                // thread_row / 4 gives us the permutation status
                //
                fragment_A[i] = sA[(N / BLOCK_WIDTH - 1) & 1][(warp_row + thread_row + i) * BLOCK_WIDTH + ((thread_row >> 2 + kFragment & 3) & 3) + (kFragment >> 2) * 4];
                fragment_A[i+4] = sA[(N / BLOCK_WIDTH - 1) & 1][(warp_row + thread_row + 16 + i) * BLOCK_WIDTH + ((thread_row >> 2 + kFragment & 3) & 3) + (kFragment >> 2) * 4];
                // Load B fragment, 8 floats
