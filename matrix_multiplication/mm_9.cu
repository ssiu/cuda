...
__global__ void __maxnreg__(128) mm_9(float* A, float* B, float* C, int N){
...
//        reinterpret_cast<float4*>(sB)[(sB_row * TILE_WIDTH + sB_col) / 4] = reinterpret_cast<float4*>(B)[(gB_row * N + gB_col) / 4];
        ...
        reinterpret_cast<float4*>(sA)[(((kBlock + 1) & 1) * TILE_WIDTH * BLOCK_WIDTH + sA_row * BLOCK_WIDTH + sA_col) >> 2] = reinterpret_cast<float4*>(A)[(gA_row * N + gA_col) >> 2];
        reinterpret_cast<float4*>(sB)[(((kBlock + 1) & 1) * TILE_WIDTH * BLOCK_WIDTH + sB_row * TILE_WIDTH + sB_col) >> 2] = reinterpret_cast<float4*>(B)[(gB_row * N + gB_col) >> 2];
