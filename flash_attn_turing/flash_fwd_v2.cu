...
//     printf("size of sQ is %d\n", size(sQ));
    ...
    Tensor sV = make_tensor(sK.data() + KV_TILE_SIZE*HEAD_SIZE, sV_layout);
    ...
    Tensor sS = make_tensor(make_smem_ptr(reinterpret_cast<float*>(&smem_[0] + array_offset)), sS_layout);
    Tensor sP_float = make_tensor(sS.data() + Q_TILE_SIZE * KV_TILE_SIZE, sS_layout);
    Tensor sO = make_tensor(sP_float.data() + Q_TILE_SIZE * KV_TILE_SIZE, sO_layout);
    Tensor sO_accum = make_tensor(sO.data() + Q_TILE_SIZE * HEAD_SIZE, sO_layout);

    float rM_old[Q_TILE_SIZE] = {-FLT_MAX, -FLT_MAX, -FLT_MAX, -FLT_MAX, -FLT_MAX, -FLT_MAX, -FLT_MAX, -FLT_MAX,
...
//     dim3 dimGrid(batch_size, num_heads, seq_len / 16);
    ...
    auto kernel = flash_fwd_v2_kernel<decltype(sQ_layout), decltype(copy_Q), decltype(mma_S),
    ...
    flash_fwd_v2_kernel<<<dimGrid, dimBlock, maxbytes>>>(q_ptr, sQ_layout, copy_Q, mma_S,
