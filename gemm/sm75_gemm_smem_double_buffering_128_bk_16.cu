...
)
{
    ...
    Tensor tAsA_1 = thr_copy_a.partition_D(sA_1);
    ...
    for (int k_tile = 0; k_tile < K_TILE_MAX; k_tile++)
    {
        ...
        if (k_tile % 2 == 0) {
            ...
            for (int k_block = 0; k_block < K_BLOCK_MAX; k_block++) {
                ...
                copy(s2r_tiled_copy_a, s2r_tCsA_1(_,_,k_block), tCrA_copy_view(_,_,k_block));
                copy(s2r_tiled_copy_b, s2r_tCsB_1(_,_,k_block), tCrB_copy_view(_,_,k_block));
...
void gemm_smem_double_buffering_128_bk_16(half_t* A, half_t* B, float* C, int m, int n, int k) {
...
//     printf("%d\n", prob_shape[2]);
    ...
    using SmemLayoutAtomB = decltype(composition(
        Swizzle<3, 3, 3>{},
        make_layout(make_shape(Int<32>{}, Int<16>{}),
    ...
    dim3 dimBlock(128);
    gemm_smem_double_buffering_128_bk_16_kernel<<<dimGrid, dimBlock>>>(prob_shape, cta_tiler,
