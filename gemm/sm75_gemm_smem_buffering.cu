...
__global__ void gemm_smem_buffering_kernel(
...
)
{
    ...
    // prologue
    // load data from gmem to smem for k_tile == 0
...
void gemm_smem_buffering(half_t* A, half_t* B, float* C, int m, int n, int k) {
...
//                         make_stride(Int<8>{}, Int<1>{}));
    ...
    dim3 dimBlock(128);
    gemm_smem_buffering_kernel<<<dimGrid, dimBlock>>>(prob_shape, cta_tiler,
