...
//             copy(tCsB(_,_,k_block), tCrB(_,_,k_block));
    ...
    //copy(tCrC, tCgC);
    //copy(tCrC, tCsC);
    //copy(tCrC(_,0,0), tCsC(_,0,0));
    for (int i=0;i<4;i++) {
        for (int j=0; j<4; j++) {
            #if 1
                if(thread0()) {
                    print("tCrC(_,i,j) : "); print(tCrC(_,i,j)); print("\n");
                }
            #endif
...
void gemm_vectorized_store_128(half_t* A, half_t* B, float* C, int m, int n, int k) {
...
//     auto kernel = &ln_fwd_kernel<Kernel_traits, IsDropoutConst, HasColscaleConst, HasSubsetConst, IsEvenColsConst>;
    auto kernel = gemm_vectorized_store_128_kernel<decltype(prob_shape), decltype(cta_tiler),
