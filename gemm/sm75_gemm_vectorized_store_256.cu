...
void gemm_vectorized_store_256_kernel(
...
void gemm_vectorized_store_256(half_t* A, half_t* B, float* C, int m, int n, int k) {
...
//     auto kernel = &ln_fwd_kernel<Kernel_traits, IsDropoutConst, HasColscaleConst, HasSubsetConst, IsEvenColsConst>;
    auto kernel = gemm_vectorized_store_256_kernel<decltype(prob_shape), decltype(cta_tiler),
    ...
    gemm_vectorized_store_256_kernel<<<dimGrid, dimBlock, maxbytes>>>(prob_shape, cta_tiler,
