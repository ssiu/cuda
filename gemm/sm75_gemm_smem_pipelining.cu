...
__global__ void gemm_smem_pipelining_256_kernel(
...
void gemm_smem_pipelining_256(half_t* A, half_t* B, float* C, int m, int n, int k) {
...
//     printf("%d\n", prob_shape[2]);
    ...
    dim3 dimBlock(256);
    gemm_smem_pipelining_256_kernel<<<dimGrid, dimBlock>>>(prob_shape, cta_tiler,
